...
class DaftarBarangIterator:
    def __init__(self, barangDagang):
        # Ubah dict menjadi list tuple supaya bisa diurutkan
        self.barangDagang = list(barangDagang.items())
        self.keys = [item[0] for item in self.barangDagang]  # Menyimpan semua key
        self.index = 0
        self.bubbleSort()

    def bubbleSort(self):
        # Implementasi Bubble Sort untuk mengurutkan barang berdasarkan nama barang
        n = len(self.barangDagang)
        for i in range(n):
            for j in range(0, n-i-1):
                # Bandingkan namaBarang dari dua elemen yang bersebelahan
                if self.barangDagang[j][1]["namaBarang"] > self.barangDagang[j+1][1]["namaBarang"]:
                    # Tukar jika salah urut
                    self.barangDagang[j], self.barangDagang[j+1] = self.barangDagang[j+1], self.barangDagang[j]

    def __next__(self):
        if self.index >= len(self.barangDagang):
            raise StopIteration()
        kodeBarang = self.barangDagang[self.index][0]
        data = self.barangDagang[self.index][1]
        self.index += 1
